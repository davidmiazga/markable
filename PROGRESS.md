...
- **Drag-to-move files and folders in file browser tree** — Completed full CRUD + reorganisation for the file browser. The drag-and-drop plumbing (`attachDragDropListeners`, `moveNode`, `.drag-over` CSS) already existed; this task hardened and completed it. Fixed `moveNode` to apply prefix-substitution loop for directory moves (all open tabs under a moved folder get their paths updated). Switched MIME key to `"text/x-markable-path"` to reject external OS/browser drags. Added file-on-file drop (resolves to parent dir). Fixed own-parent no-op guard to apply uniformly including vault-root children (removed erroneous `targetType !== "vault"` exception). Added `.is-dragging` class on drag source. Guarded `dragover` against external drags via `types.includes`. Error string formatting fix (`err.message` not `String(err)`). Extracted `resolveDropTarget` pure helper (exported for testing). 20 new Vitest tests (D1–D11 + D8b); code-reviewer approved 2026-05-01.
